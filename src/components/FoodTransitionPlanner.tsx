...
export const FoodTransitionPlanner = () => {
    ...
    const [selectedContinuedFoodId, setSelectedContinuedFoodId] = useState<number | undefined>(undefined);
    const [selectedContinuedFoodAmount, setSelectedContinuedFoodAmount] = useState<number>(0);

    const currentFood = FoodMaster.find(f => f.id === currentFoodId);
    const newFood = FoodMaster.find(f => f.id === newFoodId);
    const selectedContinuedFood = FoodMaster.find(f => f.id === selectedContinuedFoodId);

    const addContinuedFood = () => {
        if (selectedContinuedFood && selectedContinuedFoodAmount > 0) {
            setContinuedFoods([...continuedFoods, {
    ...
    const plans = currentFood && newFood ? calculateFoodTransitionPlan(
        currentFood,
        newFood,
        requiredCalories,
        transitionDays,
    ...
    return (
        <div className="p-4">
                        ...
                        >
                            ...
                            {FoodMaster.map((food) => (
                                <option key={food.id} value={food.id}>
                                    {food.name} ({food.type === FoodType.Dry ? `${food.kcalPer100}kcal/100g` : `${food.kcalPerBag}kcal/${food.gramsPerBag}g`})
                                </option>
                            ))}
                        </Select>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <Select
                            ...
                            onChange={(e) => setSelectedContinuedFoodId(Number(e.target.value))}
                            className="mt-1 block w-full"
                        >
                            <option value="">選択してください</option>
                            {FoodMaster.map((food) => (
                    ...
                    <div>
                        <label className="block text-sm font-medium">
                            {selectedContinuedFood?.type === FoodType.Wet ? '袋数' : 'グラム数'}
                        </label>
                        <input
                            type="number"
                            value={selectedContinuedFoodAmount || ''}
                            onChange={(e) => setSelectedContinuedFoodAmount(parseFloat(e.target.value))}
                    ...
                    <div className="flex items-end">
                        <Button
                            ...
                            disabled={!selectedContinuedFood || selectedContinuedFoodAmount <= 0}
                            className="w-full"
                        >
                            追加
                        </Button>
                        ...
                        <div className="space-y-2">
                            {continuedFoods.map((cf, index) => (
                                <div key={index} className="flex items-center justify-between bg-gray-50 p-2 rounded">
                                    <span>
                                        {cf.food.name} - {cf.food.type === FoodType.Wet ? `${cf.amount}袋` : `${cf.amount}g`}
                                    </span>
                                    <Button
                                        color="failure"
                                        size="xs"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div>
                    <label className="block text-sm font-medium">必要カロリー (kcal/日)</label>
                    <input
                        type="number"
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">日目</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">現在のフード (g)</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">新しいフード (g)</th>
                            {plans[0]?.continuedFoods.map((cf, index) => (
                                <th key={index} className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    継続フード {index + 1} (g)
                                </th>
                            ))}
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">合計カロリー</th>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">新フードの割合 (%)</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                        {plans.map((plan) => (
                            <tr key={plan.day}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">{plan.currentFoodGrams}</td>
                                <td className="px-6 py-4 whitespace-nowrap">{plan.newFoodGrams}</td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">{plan.newFoodPercentage}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
